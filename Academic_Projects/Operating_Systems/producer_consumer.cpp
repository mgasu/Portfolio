...
//  
//
//  Created by Maria Garcia on 10/14/23.
//
...
int buffSize = MAX_BUFFER_SIZE;
int prod = MAX_NO_OF_PRODUCERS;
int cons = MAX_NO_OF_CONSUMERS;
int uuid = -1; //user ID, initialized to -1

// Then use module_param to pass them from insmod command line. (--Assignment 2)
module_param(buffSize, int, S_IRUSR | S_IRUSR | S_IRGRP | S_IWGRP);
MODULE_PARM_DESC(buffSize, "Buffer Size");

module_param(prod, int, S_IRUSR | SIWUSR | S_IRGRP | S_IWGRP);
MODULE_PARM_DESC(prod, "Number of Producers");

module_param(cons, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
MODULE_PARM_DESC(cons, "Number of Consumers");

module_param(uuid, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
...
int producer_thread_function(void *pv)
{
    ...
    for_each_process(task)
    {
        if (task->cred->uid.val == uuid)
        {
            ...
            buffer[fill].boot_time = ktime_get_ns - task->start_time;
...
int consumer_thread_function(void *pv)
{
    ...
    while (!kthread_should_stop())
    {
        ...
        down(&full);
        // acquire the mutex for exclusive access to the shared buffer
        down(&mutex);
        ...
        //consume the process information from the buffer
        ...
        //release the mutex after consuming the process info
        up(&empty);
        
        //signal there's a free spot in the buffer
        up(&empty);
...
static int __init thread_init_module(void)
{
    ...
    if (buffSize > 0 && (prod >= 0 && prod < 2))
    {
        ...
        sema_init(&mutex, MAX_BUFFER_SIZE);
        sema_init(&full, MAX_BUFFER_SIZE);
        sema_init(&empty, MAX_BUFFER_SIZE);
        ...
        for (int index = 0; index < prod; index++) {
                   ctx_producer_thread[index] = kthread_run(producer_thread_function, NULL, producer_thread_name[index]);
                   if (IS_ERR(ctx_producer_thread[index])) {
                       PCINFO("Failed to create producer thread %d", index);
                       ctx_producer_thread[index] = NULL;
                   }
               }
        
        // Use kthread_run to create consumer kernel threads here
               for (int index = 0; index < cons; index++) {
                   ctx_consumer_thread[index] = kthread_run(consumer_thread_function, NULL, consumer_thread_name[index]);
                   if (IS_ERR(ctx_consumer_thread[index])) {
                       PCINFO("Failed to create consumer thread %d", index);
                       ctx_consumer_thread[index] = NULL;
                   }
               }
